...
ChiliBook.recipes[ "nginx.js" ] = 
{
    steps: {
          com   : { exp: /(?:#.*)/ }
  		, regex  : { exp: /(!?~\s*)(([\"\'])(?:(?:[^\3\\]*?(?:\3\3|\\.))*?[^\3\\]*?)\3|[\S]+\$|[\S]+)/
  			        , replacement: '$1<span class="$0">$2</span>' }
        , string  : { exp: /(?:\'[^\'\\\n]*(?:\\.[^\'\\\n]*)*\')|(?:\"[^\"\\\n]*(?:\\.[^\"\\\n]*)*\")/ }
        , keyword   : { exp: /\b(add_header|array_join|array_map|array_map_op|array_split|break|default_type|drizzle_keepalive|drizzle_pass|drizzle_query|drizzle_server|echo|echo_before_body|echo_end|echo_exec|echo_foreach_split|echo_location|echo_location_async|error_page|eval|eval_buffer_size|eval_override_content_type|eval_subrequest_in_memory|events|http|if|index|internal|keepalive|listen|location|memc_pass|postgres_pass|postgres_query|postgres_server|proxy_pass|proxy_set_header|rds_json|resolver|return|rewrite|root|server|server_name|set|set_hashed_upstream|set_if_empty|set_quote_sql_str|set_unescape_uri|upstream|upstream_list|worker|worker_connections|worker_processes|xss_as_str|xss_callback_arg|xss_get|xss_input_types)\b/ }
    }
};
...
ChiliBook.recipes[ "javascript.js" ] = 
{
	steps: {
		...
		, keyword: { exp: /\b(arguments|break|case|catch|continue|default|delete|do|else|false|for|function|if|in|instanceof|new|null|return|switch|this|true|try|typeof|var|void|while|with)\b/ }
